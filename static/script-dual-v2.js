...
// qrBoxSize is now defined in HTML before this script loads
...
async function initCamera(cameraIndex) {
    ...
    try {
        ...
        // Camera config - use fixed qrBoxSize from HTML
        console.log('=== DEBUG: qrBoxSize from HTML:', qrBoxSize);
        const config = {
            fps: 10,
        ...
        if (idx !== currentIndex) {
            html += `
                <button onclick="useCameraIndex(${currentIndex}, ${idx})" 
                        style="display: block; margin: 10px auto; padding: 12px 20px; background: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer; min-width: 1000px; text-align: left;">
